...
└─────────────────────────────────────────────────────────────┘
          ...
          ▼                             ▼
┌────────────────────┐       ┌──────────────────────────────┐
│  git-eternal-data  │       │  apps/web (Vercel)           │
...
2. **Token auth**: `HARVEST_TOKEN` (classic PAT with `repo` scope) reads GitHub traffic API; `VAULT_TOKEN` writes to `git-eternal-data`
...
6. **Storage**: JSON files written to `git-eternal-data` branch under `data/{owner}/{repo}/{year}/{YYYY-MM}.json`
...
## Purpose of `harvest` and `git-eternal-data`
...
- **`git-eternal-data`**: A separate GitHub repo (or orphan branch) that acts as a flat-file database — contains `index.json` (summary), `harvest_log.json` (run history), `config.json` (tracked repos), and `data/` tree (raw monthly ledgers)
